...
const TaskCard = ({ task, onClick }: { task: Task; onClick: () => void }) => {
  return (
    <Card
      onClick={onClick}
      className="hover:shadow-md transition-all hover:translate-y-1 duration-300 cursor-pointer"
    >
      <CardHeader>
        <div className="flex justify-between items-center">
          <Badge
      ...
      <CardContent>
        ...
        {task.description && (
          <p className="mb-2 text-muted-foreground text-sm line-clamp-2">
            {task.description}
          </p>
        )}
            ...
            {task.assignees && task.assignees.length > 0 && (
              <div className="flex -space-x-2">
                {task.assignees.slice(0, 5).map((member) => (
                  <Avatar
                    key={member._id}
                    className="relative bg-gray-700 border-2 border-background rounded-full size-8 overflow-hidden"
                    title={member.name}
                  >
